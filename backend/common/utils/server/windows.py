...
def WriteReg(path, key, value, type=winreg.REG_SZ):
    ...
    try:
        newKey = winreg.OpenKey(
            winreg.HKEY_LOCAL_MACHINE, path, 0, winreg.KEY_ALL_ACCESS
        )
    except:
...
def ExecShell(cmdstring, cwd=None, timeout=None, shell=True):
    ...
    try:
        ...
        succ_f = tempfile.SpooledTemporaryFile(
            max_size=4096000, mode="wb+", suffix="_succ", prefix="btex_" + rx
        )
        err_f = tempfile.SpooledTemporaryFile(
            max_size=4096000, mode="wb+", suffix="_err", prefix="btex_" + rx
        )

        sub = subprocess.Popen(
            cmdstring, shell=shell, bufsize=128, stdout=succ_f, stderr=err_f
        )
        sub.wait()
            ...
            if format in ["str", "string", "s"]:
                ...
                if format == "ipv4":
                    ...
                    if not is_ipv4(result) and not is_ipv6(result):
                        raise ValueError(
                            "参数：{}，要求正确的ipv4/ipv6地址".format(key)
                        )
            elif format in ["w", "letter"]:
                if not re.match(r"^\w+$", result):
                    raise ValueError(
                        "参数：{}，要求只能是英文字母或数据组成".format(key)
                    )
...
def GetNetWork():
    cache_timeout = 86400
    # otime = await redis.session.get("otime")
    ntime = time.time()
    networkInfo = {}
    networkInfo["network"] = {}
    ...
    for net_key in networkIo_list.keys():
        if (
            net_key.find("Loopback") >= 0
            or net_key.find("Teredo") >= 0
            or net_key.find("isatap") >= 0
        ):
            continue
        ...
        # await redis.session.set(otime_key, otime, cache_timeout)
