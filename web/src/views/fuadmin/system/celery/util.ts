...
export function getCrontabData(val) {
  ...
  if (val.month_of_year !== '*') {
    ...
    if (val.day_of_month === '?') {
      res = `每周${val.day_of_week === 'L' ? '最后一' : val.day_of_week} ${val.hour}点${val.minute}分`;
    } else {
      res = `每月 ${val.day_of_month === 'L' ? '最后一' : val.day_of_month} 日 ${val.hour}点${val.minute}分`;
    }
    res = `每${val.day_of_week === '?' ? '日' : week[val.day_of_week] || val.day_of_week} ${val.hour}点${val.minute}分 `;
  } else if (val.hour !== '*') {
